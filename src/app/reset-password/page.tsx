...
export default function ResetPasswordPage() {
  ...
  async function handleVerify(e: React.FormEvent) {
    ...
    setLoading(true);
    const { ok, error } = await verifyResetCode(email, code);
    setLoading(false);
    if (!ok) {
  ...
  async function handleReset(e: React.FormEvent) {
    ...
    setLoading(true);
    const { ok, error } = await resetPassword(email, code, password);
    setLoading(false);
    if (!ok) {
            ...
            <>
              ...
              <form onSubmit={handleReset} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  <Label htmlFor="password" style={{ color: 'var(--fg-2)', fontSize: '13px' }}>New password</Label>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  <Label htmlFor="confirmPassword" style={{ color: 'var(--fg-2)', fontSize: '13px' }}>Confirm password</Label>
