...
import { createScriptApi, suggestExtractionFields, optimizeScript, generateScript } from '@/lib/api';
...
export default function ScriptCreatePage() {
  ...
  const [form, setForm] = useState({ name: '', language: 'ur', content: '' });
  ...
  async function handleSuggest() {
    if (!form.content.trim()) { setError('Write the script content first, then click Suggest.'); return; }
    setSuggesting(true);
    setError('');
  ...
  async function handleOptimize() {
    if (!form.content.trim()) { setError('Write the script content first, then click Optimize.'); return; }
    setOptimizing(true);
    setError('');
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError('');
    setSaving(true);
    const result = await createScriptApi({
      name: form.name.trim(),
    ...
    if (result) {
      ...
      setError('Failed to create script. Check the API and try again.');
    }
  }
  ...
  return (
    <main className="flex flex-1 flex-col gap-5 p-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-semibold text-foreground">New Script</h1>
          <p className="text-sm text-muted-foreground">Write a call script for your agents</p>
        </div>
      </div>
          ...
          <div className="flex flex-col gap-5">
            ...
            <Card>
              <CardHeader className="border-b">
                ...
                <CardDescription>Name and language of the script</CardDescription>
              </CardHeader>
              <CardContent className="pt-4 grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5">
                  <Label htmlFor="name" className="text-xs text-muted-foreground">Script Name *</Label>
                  <Input id="name" placeholder="e.g. Restaurant Ordering Script" value={form.name} onChange={e => set('name', e.target.value)} />
                </div>
                <div className="flex flex-col gap-1.5">
            ...
            <Card>
              <CardHeader className="border-b">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <CardDescription>Full script — menu, FAQs, pricing, flow. Agent uses this via RAG.</CardDescription>
                  </div>
              ...
              <CardContent className="pt-4">
                <Textarea
                  rows={22}
                  placeholder="Write your script here. Use == Headings == and list items clearly."
                  value={form.content}
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <CardDescription className="mt-1">
                    Data the AI will extract from every call transcript automatically.
                  </CardDescription>
                </div>
                <Button
                  type="button"
